...
from correlation_coefficients import cal_correlation_coefficients
from correlation_coefficients import draw_security_evaluation
    ...
    if PERMUTE_TWICE:
        ...
        for i in range(len(images)):
            ...
            if size_threshold_lower <= len(img[0]) < size_threshold:
                ...
                cv2.imwrite(image_folder_res + str(i) + file_suffix_res, img)
        draw_security_evaluation(times_permute_twice, cor_score_permute_twice, 'mediumblue', '3 time permute')
...
plt.xlabel('sec/face')
plt.ylabel('(1 - correlation coefficients)')
plt.show()
